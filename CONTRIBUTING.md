...
`domdf_spreadsheet_tools` uses `tox` to automate testing, autoformatting and packaging.

For more information on installing `tox` see https://tox.readthedocs.io/en/latest/install.html

## Coding style

`domdf_spreadsheet_tools` uses `yapf` for code formatting and `isort` to sort imports.
