...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  do
    ...
    autopep8 --in-place --select "$error" -a --recursive domdf_spreadsheet_tools/
    >&2 flake8 --select "$error" domdf_spreadsheet_tools/
  ...
  for warning in "${warnings[@]}"; do
    ...
    >&2 flake8 --select "$warning" domdf_spreadsheet_tools/
